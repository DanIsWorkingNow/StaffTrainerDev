 ...
 * This is used by other server functions to check permissions
 */
 ...
 */
export const requireRole = createServerFn({ method: 'GET' }).handler(
 ...
 * @returns boolean
 */
export const hasPermission = createServerFn({ method: 'GET' }).handler(
  async (resource: string, action: Permission['action']): Promise<boolean> => {
    const userData = await getCurrentUserRole()
 ...
 */
export const requirePermission = createServerFn({ method: 'GET' }).handler(
  async (resource: string, action: Permission['action']): Promise<boolean> => {
